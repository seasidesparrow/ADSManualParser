    ...
    def _get_bibcode(self, bibstem=None, volume=None):
        try:
            ...
            if not self.output.get("bibcode", None):
                self.output["bibcode"] = bibgen.make_bibcode(
                    self.data, bibstem=bibstem, volume=volume
                )
        except Exception as err:
    ...
    def _special_handling(self, bibstem=None):
        ...
        if bibstem == "pds..data" or bibstem == "pdss.data":
            ...
            if self.data.get("title", {}).get("textEnglish", None):
                (circular_number, circular_title) = (
                    self.data.get("title", {}).get("textEnglish").split(":")
                )
                ...
                self.output["publication"] = (
                    "Minor Planet Electronic Circ., No. %s" % circular_number
                )
